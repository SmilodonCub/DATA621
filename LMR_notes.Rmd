...
When errors are dependent, we can use generalized least squares (GLS). When errors in independent but not identically distributed, we can use weighted least squares (WLS), which is a special case of GLS

### Generalized Least Squares
Sometimes errors have non-constant variance or are correlated. Ex: may find evidence of serial correlation by looking at the residuals as in this global warming dataset

```{r}
data( globwarm, package = 'faraway' )
gwarm_mod <- lm( nhtemp ~ wusa + jasper + westgreen + chesapeake + 
                   tornetrask + urals + mongolia + tasman, 
                 data = globwarm )
sumary( gwarm_mod )
```
```{r}
cor( residuals( gwarm_mod)[-1], residuals( gwarm_mod )[-length( residuals( gwarm_mod ))])
```
use a GLS fitting function:
```{r}
gwarm_glmod <- gls( nhtemp ~ wusa + jasper + westgreen + chesapeake + 
                   tornetrask + urals + mongolia + tasman, 
...
```{r}
intervals( gwarm_glmod, which = "var-cov")
```
There is significant positive correlation  

**Compound Symmetry Assumption**: Another example is where observations within blocks might have errors which are correlated
```{r}
data( oatvar, package = 'faraway' )
oat_glmod <- gls( yield ~ variety, oatvar, correlation = corCompSymm( form = ~1 | block ) )
intervals( oat_glmod )
```
other examples of correlated error - spatial data where the relative locations of the observations can be used to model the error correlation.


### Weighted Least Squares
WLS can be used in special cases where the errors are uncorrelated, but have unequal variance where the form of the inequality is known.  

French Election data:
```{r}
data( fpe, package = 'faraway' )
vote_mod <- lm( A2 ~ A + B + C + D + E + F + G + H + J + K + N-1, fpe, weights = 1/EI )
coef( vote_mod )
```

Solution to the constrained least squares problem which requires 0:1
```{r}
M <- list( w=1/fpe$EI, X = model.matrix( vote_mod ), y = fpe$A2, 
           Ain = rbind(diag(11),-diag(11)),
           C = matrix(0,0,0), array(0,0), S=list(),
           off=NULL, p=rep(0.5,11), bin = c(rep(0,11),rep(-1,11)))
a <- pcls( M )
names( a ) <- colnames( model.matrix( vote_mod ) )
round( a,3 )
```


### Testing for Lack of Fit
How can we tell whether a model fits the data? We need to make a comparison to some model-free estimate of $\sigma^2$

```{r}
data( corrosion, package = 'faraway' )
plot( loss ~ Fe, corrosion, xlab = "Iron content", ylab = "Weight loss" )
corrosion_lmod <- lm( loss ~ Fe, data = corrosion )
abline( coef( corrosion_lmod ) )
```
fit a model that researves a parameter for each group of data with the same value of x
```{r}
corrosion_group_lmod <- lm( loss ~ factor( Fe ), data = corrosion )
plot( loss ~ Fe, corrosion, xlab = "Iron content", ylab = "Weight loss" )
points( corrosion$Fe, fitted( corrosion_group_lmod ), pch = 3 )
abline( coef( corrosion_lmod ) )
anova( corrosion_lmod, corrosion_group_lmod )
```

Fit the same data with a 6th order polynomial
```{r}
corrosion_6ord <- lm( loss ~ Fe + I(Fe^2) + + I(Fe^3) + + I(Fe^4) + + I(Fe^5) + + I(Fe^6),
                      data = corrosion )
plot( loss ~ Fe, data=corrosion, ylim= c(60,130))
points( corrosion$Fe, fitted( corrosion_group_lmod ), pch=3 )
grid <- seq( 0,2,len=50 )
lines( grid, predict( corrosion_6ord, data.frame( Fe=grid)))
```
```{r}
summary( corrosion_6ord )$r.squared
```
The fit is fantastic! wait....this is ridiculous


### Robust Regression
Compare Least Squares to Huber's method  
```{r}
data( gala, package = 'faraway' )
gal_lsmod <- lm( Species ~ Area + Elevation + Nearest + Scruz + Adjacent, data = gala )
sumary( gal_lsmod )
```
```{r}
gal_rlmod <- rlm( Species ~ Area + Elevation + Nearest + Scruz + Adjacent, data = gala )
summary( gal_rlmod )
```
```{r}
wts <- gal_rlmod$w
...
LAD regression
```{r}
gal_LADmod <- rq( Species ~ Area + Elevation + Nearest + Scruz + Adjacent, data = gala )
summary( gal_LADmod )
```

Least Trimmed Squares - for really big large errors (poorly entered data?)
```{r}
set.seed( 123 )
gal_ltsmod <- ltsreg( Species ~ Area + Elevation + Nearest + Scruz + Adjacent, data = gala )
coef( gal_ltsmod )
```
Bootstrapped LTS
```{r}
...
for( i in 1:1000 ){
  newy <- predict( gal_ltsmod ) + residuals( gal_ltsmod )[sample( 30, rep=T)]
...
```{r}
colnames( bcoef ) <- names( coef( gal_ltsmod ) )
...
p1 <- ggplot( bcoef, aes( x = Area ) ) +
  geom_density() +
  xlim( 1.45, 1.65 ) +
  geom_vline( xintercept = c( 1.4976, 1.6230 ), linetype = "dashed" )
p2 <- ggplot( bcoef, aes( x = Adjacent ) ) +
  geom_density() +
  xlim( -0.25, -0.13 ) +
  geom_vline( xintercept = c( -0.23375, -0.15138 ), linetype = 'dashed' )

grid.arrange( p1, p2, ncol = 2 )
```
Summary  

* Robust estimators provide protection against long-tailed error
* Robust estimates supply coefficients and possibly SE by way of bootstrap or other methods
* Robust methods can be used in addition to least squares as a confirmatory method
* Robust estimates are useful when data need to be fit automatically
