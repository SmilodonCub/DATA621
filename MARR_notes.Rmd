...
## Variable Selection
choosing the 'best' model from a class of multiple regression models using what are called variable selection methods.  

In general, the more predictor variables included in a valid model the lower the bias in the predictions, but the higher the variance. Including too many variables in a regression model is commonly called over-fitting while the opposite is called under-fitting.

*"If the main interest is in finding an interpretable model or in identifying the 'true' underlying model as closely as possible, prediction accuracy is of secondary importance (to variable selection)....On the other hand, if prediction is the focus of interest, it is usually acceptable for the selected model to contain some extra variables, as long as the coefficients of those variables are small"*

### Evaluating Potential Subsets of Predictor Variables
4 criteria to evaluate subsets of predictor variables 

#### the $R^2_{adj}$ criterion
Choosing the subset of predictors that has the highest values of $R^2_{adj}$ tends towards over-fitting

#### AIC, Akaike's Information Criterion
The smaller the value of AIC, the better the model

#### $\mbox{AIC}_C$: Corrected AIC
a bias corrected version of AIC that has a larger penalty for model complexity: good for small sample sizes

#### BIC: Bayesian Information Criterion
the smaller the value of BIC, the better. higher penalty for model complexity that AIC.

**What to do?!?**: a popular data strategy is to calculate $R^2_{adj}$, AIC, $\mbox{AIC}_C$, and BIC and compare the results for models then select by which minimizes AIC, $\mbox{AIC}_C$, and BIC and maximizes $R^2_{adj}$.


### Deciding on the Collection of Potential Subsets of Predictor Variables

#### All possible subsets
Consider all $2^m$ possible equations and identify the subsets of the predictors of a given size that maximizes a measure of fit or minimizes an information criterion

Example: Bridge Construction
```{r}
bridge <- read.csv( 'bridge.csv', sep = '\t' )
glimpse( bridge )
```
```{r}
bridge_mod <- lm( data = bridge, log( Time ) ~ log( DArea ) + 
                    log( CCost ) + log( Dwgs ) + log( Length ) + log( Spans ) )
bridge_mod_sum <- summary( bridge_mod )
bridge_mod_sum
```
The model is statistically highly significant, but not all the features are.

#### Stepwise Subsets
**Backwards elimination** - start with all predictors in the regression model and successively delete predictors that have either the lower information criterion or the highest p-value until either all predictors are removed or the information criterion increases.

```{r}
backwards_bridge <- stepAIC( bridge_mod, direction = 'backward' )
backwards_bridge
```

**Forward selection** - start with nothing. at each step, add a predictor and select the mod with the lowest p-val/highest information criterion. repeat until there are no more variables to add or the information decreases/p-val increases.
```{r}
bridge_mod2 <- lm( log( Time ) ~ 1, data=bridge)
forwards_bridge <- stepAIC( bridge_mod, direction = 'forward' )
forwards_bridge
```



### Assession the Predictive Ability of Regression Models
Assess the predictive ability of different regression models by evaluating their performance on a new data set.  
To do this, randomly split the data into a training data set and a testing data set.  

Example: predicting the log(cancer volume), or `lacavol` from assorted prostate cancer feature variables.  




#### STEP 1: Model Building Using the Training Data Set
```{r}
prostateTraining <- read.csv( 'prostateTraining.csv' , sep = '\t' )
glimpse( prostateTraining )
```
...
There aren't any obvious signs of nonlinearity, so move forward with a kitchen sink model:
```{r}
prostate_mod <- lm( lpsa ~ lcavol + lweight + age + lbph + svi + lcp + gleason + pgg45, data = prostateTraining )
summary( prostate_mod )
```

Now to look at a scatterplot of the standardized residuals for each predictor....
```{r}
standard_residuals_prostate <- rstandard( prostate_mod )
glimpse( standard_residuals_prostate )
prostateTraining_res <- cbind( prostateTraining, standard_residuals_prostate, fitted( prostate_mod ) )
glimpse( prostateTraining_res )
```

```{r}
prostateTraining_res_plot <- prostateTraining_res %>%
  pivot_longer( cols = c( lcavol, lweight, age, lbph,  svi,  lcp,  gleason, pgg45, `fitted(prostate_mod)` ), names_to = 'names', values_to = 'val' )

ggplot( data = prostateTraining_res_plot, aes( x = standard_residuals_prostate, y = val ) ) +
  geom_point( fill = NA, shape = 21, alpha = 0.5, size = 3 ) +
  facet_wrap( ~ names, scales = 'free' ) +
  theme_classic()
```
Random patterns, no sign of nonlinear trend.  
OK, now to look at the target variable against the fitted values:
```{r}
ggplot( data = prostateTraining_res, aes( x = `fitted(prostate_mod)`, y = lpsa ) ) +
  geom_point( fill = NA, shape = 21, alpha = 0.5, size = 3 ) +
  geom_smooth( method = 'lm', se = F, col = 'black' ) +
  theme_classic()
```

We see that there is a reasonable linear fit.  
Now to plot the model diagnostics: 
```{r}
plot( prostate_mod )
```
Diagnostics appear reasonable....

Now the marginal model plots: 
```{r message=F}
mmps( prostate_mod )
```
The curves in the marginal model plot match pretty well; this supports the model's validity  

Now to look at the added variable plots
```{r}

avPlots( prostate_mod, marginal.scale=TRUE, ellipse=list(levels=0.5)) 
```
Case 45 stands out for `lweight` & should be investigated  
Otherwise, let's go ahead and assume this is a valid model.  

Compute the variance inflation factor
```{r}
vif( prostate_mod )
```
We can see that none of these are higher than 5, so we don't need to be concerned about colinearity

see what results from stepAIC elimination
```{r}
stepAIC( prostate_mod, direction = 'backward' )
```
The best AIC likes using 7 predictors: lpsa ~ lcavol + lweight + age + lbph + svi + lcp + pgg45

Try AICc  ?not working?
```{r}
AICc( prostate_mod )
AICc( lm( lpsa ~ lcavol + lweight + age + lbph + svi + lcp + pgg45, data = prostateTraining ) )
AICc( lm( lpsa ~ lcavol + lweight + lbph + svi + lcp + pgg45, data = prostateTraining ) )
AICc( lm( lpsa ~ lcavol + lweight + lbph + svi + pgg45, data = prostateTraining ) )
AICc( lm( lpsa ~ lcavol + lweight + lbph + svi, data = prostateTraining ) )
AICc( lm( lpsa ~ lcavol + lweight + svi, data = prostateTraining ) )
AICc( lm( lpsa ~ lcavol + lweight, data = prostateTraining ) )
AICc( lm( lpsa ~ lcavol, data = prostateTraining ) )
```
corrected AIC prefers 4 terms: lpsa ~ lcavol + lweight + lbph + svi

```{r}
stepAIC( prostate_mod, k = log( nrow( prostateTraining ) ) )
```
The BIC prefers just 2 predictors: lpsa ~ lcavol + lweight

Let's look at the summaries for both of these 2 models:
```{r}
summary( lm( lpsa ~ lcavol + lweight + age + lbph + svi + lcp + pgg45, data = prostateTraining ) )
```
```{r}
summary( lm( lpsa ~ lcavol + lweight + lbph + svi, data = prostateTraining ) )
```



```{r}
summary( lm( lpsa ~ lcavol + lweight, data = prostateTraining ) )
```
ready for next step: how do these perform on new data....

#### STEP 2: Model Comparison Using the Test Data 
```{r}
prostateTest <- read.csv( 'prostateTest.csv' , sep = '\t' )
glimpse( prostateTest )
```


Try each of the models with the test data:
```{r}
summary( lm( lpsa ~ lcavol + lweight + age + lbph + svi + lcp + pgg45, data = prostateTest ) )
```
```{r}
summary( lm( lpsa ~ lcavol + lweight + lbph + svi, data = prostateTest ) )
```
```{r}
summary( lm( lpsa ~ lcavol + lweight, data = prostateTest ) )
```
YIKES!, none of this looks very convincing....  

....now to revisit the outlier of CASE45
most of this issue is due to the dataset being too small. would need to really evaluate the outliers like case45. Is it a 'real' datapoint? can you justify eliminating it?....


### Recent Development in Variable Selection - LASSO
**L**east **A**bsolute **S**hrinkage and **S**election **O**perator  
LASSO performs variable selection and regression coefficient estimation simultaneously.  
ise BIC to find the optimal LASSO model when sparsity is a concern.  
...
WLS - an alternative way to cope with nonconstant error variance.

### Straight-Line Regression Based on WLS
```{r}
cleaning <- read.csv( 'cleaning.csv', sep = '\t' )
glimpse( cleaning )
```
...
```
```{r}
wts <- 1/fitted( lm( abs( residuals( cleaning_lm ) ) ~ cleaning$Crews ) )^2
cleaning_wgtlm <- lm( Rooms ~ Crews, weights = wts, data = cleaning )
summary( cleaning_wgtlm )
```

Predicting Intervals for Weighted Least Squares
$$\sqrt{(\hat{\sigma^2/w * \mbox{sefit}(y|X=x^*)^2})}$$
Leverage for WLS ... if all weights are equal WLS = LS (what's the point of this exercise)

Using least squares to calculate weights least squares...  

The use of weighted least squares: best in special cases where $Y_i$ is the average or median of $n_i$ observations such that $Var(Y_i)\propto \frac{1}{n_i}$. In this case we take $w_i=n_i$.


## Serially Correlated Errors

**Serial Correlation** - results from the current time period are correlated with results from earlier time periods.

### Autocorrelation

```{r}
confood2 <- read.csv( 'confood2.csv', sep = '\t' )
glimpse( confood2 )
```
```{r}
p1 <- ggplot( data = confood2, aes( x = log( Price ), y = log( Sales ), col = factor( Promotion ) ) ) +
  geom_point() +
  theme_classic()
p2 <- ggplot( data = confood2, aes( x = Week, y = log( Sales ) ) ) +
  geom_line() +
  geom_point( aes( x = Week, y = log( Sales ), col = factor( Promotion ) ) ) +
  theme_classic()
grid.arrange( p1, p2, ncol = 1 )
```
Fig1 - gives the appearance that there is a linear relationship between log(Sales) and log(Price)
Fig2 - log( Sales ) in week t are positively correlated with log( Sales ) in week t-1

```{r}
confood2$SalesLag1 <- NA
confood2$SalesLag1[1:51] <- confood2$Sales[2:52]
ggplot( data = confood2, aes( x = log( Sales ), y = log( SalesLag1 ), na.rm = T ) ) +
  geom_point( ) +
  theme_classic()
```

Fig3 - **lag 1 autocorrelation** there is a positive correlation between log(Sales) in week t and log(Sale) in week t-1

looking at the rest of the lags:
```{r}
acf( confood2$Sales, lag.max = 20, plot = T )
```
We can see from the figure above that lag1 is over 2 SE, so the previous week's sale affect the next

#### What happens if you ignore autocorrelation in the data:
```{r}
confood2_lm <- lm( data = confood2, log( Sales ) ~ log( Price ) + Week )
confood2_lm_res <- rstandard( confood2_lm )
confood2_lm_res_vals <- cbind( confood2, confood2_lm_res, fitted( confood2_lm ) )
glimpse( confood2_lm_res_vals )
```

```{r}
confood2_lm_res_plot <- confood2_lm_res_vals %>%
  mutate( Price == log( Price ) ) %>%
  pivot_longer( cols = c( Week, Sales, Price, Promotion, SalesLag1, `fitted(confood2_lm)` ), names_to = 'names', values_to = 'val' ) 
glimpse( confood2_lm_res_plot )

ggplot( data = confood2_lm_res_plot, aes( y = confood2_lm_res, x = val ) ) +
  geom_point( fill = NA, shape = 21, alpha = 0.5, size = 3 ) +
  facet_wrap( ~ names, scales = 'free' ) +
  theme_classic()
```
...
Generalized Least Squares - allows errors to be autocorrelated
Using Least Squares and ignorring autocorrelation when it exists will result in consistent estimates of $\beta_1$ but incorrect estimates of the variance of $\hat{\beta}_{ILS}$ invalidating resulting confidence intervals and hypothesis tests.

```{r}
confood2_gls <- gls( log( Sales ) ~ log( Price ) + Week + Promotion, 
                     data = confood2, 
                     correlation = corAR1(form =~Week),
                     method = "ML" )
summary( confood2_gls )
```

```{r}
acf( confood2_gls$residuals, lag.max = 20, plot = T )
```

#### A General Approach to Transforming GLS into LS
I could not figure out how to do this from the text
```{r}
...
identify(xstar[,1],ystar,confood2$Week)
...
identify(xstar[,4],ystar,confood2$Week)
```
...
identify(xstar[,4],StanRes1,labels=confood2$Week,cex=0.75)
...
par(mfrow=c(2,2))
plot(m1tls)
abline(v=2*4/length(confood2$Week),lty=2)
```


### Case Study
```{r}
bayArea <- read.csv( 'BayArea.csv', sep = '\t' )
glimpse( bayArea )
```
Matrix Plot: There are striking nonlinear patterns among the predictors
```{r}
pairs( ~ InterestRate + LoansClosed + VacancyIndex, data = bayArea )
```
Consider a naive linear model:
```{r}
bayArea_lm <- lm( data = bayArea, InterestRate ~ LoansClosed + VacancyIndex )
summary( bayArea_lm )
plot( bayArea_lm )
```

```{r}
StanRes2 <- rstandard( bayArea_lm )
par(mfrow=c(1,1))
acf(StanRes2,main="Series Standardized LS Residuals")
```

```{r}
bayArea_lm_res <- rstandard( bayArea_lm )
bayArea_lm_res_vals <- cbind( bayArea, bayArea_lm_res, fitted( bayArea_lm ) )
glimpse( bayArea_lm_res_vals )
bayArea_lm_res_plot <- bayArea_lm_res_vals %>%
  dplyr::select( LoansClosed, VacancyIndex, InterestRate, bayArea_lm_res, `fitted(bayArea_lm)` ) %>%
  pivot_longer( cols = c( LoansClosed, VacancyIndex, InterestRate, `fitted(bayArea_lm)` ), names_to = 'names', values_to = 'val' ) 
glimpse( bayArea_lm_res_plot )

ggplot( data = bayArea_lm_res_plot, aes( y = bayArea_lm_res, x = val ) ) +
  geom_point( fill = NA, shape = 21, alpha = 0.5, size = 3 ) +
  facet_wrap( ~ names, scales = 'free' ) +
  theme_classic()
```

Now a fit assuming AR(1)
```{r}
bayArea_gls <- gls( InterestRate ~ LoansClosed + VacancyIndex, 
                     data = bayArea, 
...
Now to for transformed versions of the variables:
```{r}
#R output on page 318
g <- lm( InterestRate ~ LoansClosed + VacancyIndex, data=bayArea )
rho <- 0.9572093
x <- model.matrix(g)
Sigma <- diag(length(bayArea$Month))
Sigma <- rho^abs(row(Sigma)-col(Sigma))
sm <- chol(Sigma)
smi <- solve(t(sm))
xstar <- smi %*% x
ystar <- smi %*% log(bayArea$InterestRate)
m1tls_2 <- lm(ystar ~ xstar-1) 
summary(m1tls_2)
```
```{r}
par(mfrow=c(2,2))
plot(xstar[,1],ystar,xlab="Intercept*",ylab="InterestRate*")
identify(xstar[,1],ystar,bayArea$Month)
plot(xstar[,2],ystar,xlab="LoansClosed*",ylab="InterestRate*")
identify(xstar[,2],ystar,bayArea$Month)
plot(xstar[,3],ystar,xlab="VacancyIndex*",ylab="InterestRate*")
identify(xstar[,3],ystar,bayArea$Month)
plot(xstar[,2],xstar[,3],xlab="LoansClosed*",ylab="VacancyIndex*")
identify(xstar[,2],xstar[,3],bayArea$Month)
```

```{r}
StanRes1 <- rstandard(m1tls_2)
par(mfrow=c(2,2))
acf(StanRes1,main="Standardized LSResiduals")
plot(xstar[,2],StanRes1,ylab="Standardized LS Residuals",xlab="LoansClosed*")
identify(xstar[,2],StanRes1,bayArea$Month)
plot(xstar[,3],StanRes1,ylab="Standardized LS Residuals",xlab="VacancyIndex*")
identify(xstar[,3],StanRes1,bayArea$Month)
plot(m1tls_2$fitted.values,StanRes1,ylab="Standardized Residuals",xlab="Fitted Values*")
identify(m1tls_2$fitted.values,StanRes1,bayArea$Month)
```
